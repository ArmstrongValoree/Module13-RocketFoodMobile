...
public class OrderApiController {
        ...
        if (type == null && id == null) {
            ...
            switch (type.toLowerCase()) {
                ...
                case "courier":
                    ...
                    break;
                case "status_id":
                    if (!orderService.orderStatusExists(id)) {
                        return ResponseEntity.status(HttpStatus.UNPROCESSABLE_ENTITY)
                                .body(Collections.singletonMap("error", "Invalid status id"));
                    }
                    orders = orderService.getOrdersByOrderStatusId(id);
                    break;
                default:
                    return ResponseEntity.status(HttpStatus.UNPROCESSABLE_ENTITY)
    ...
    public ResponseEntity<Object> updateOrderCourier(
            @PathVariable("id") int orderId,
            @RequestBody Map<String, Integer> courierData) {

        // Validate courier_id
        if (courierData == null || !courierData.containsKey("courier_id")) {
            return ResponseEntity.badRequest().body(Map.of(
                "error", "Bad request",
                "details", "courier_id is required"
            ));
        }

        Integer courierId = courierData.get("courier_id");
        
        // Validate courier exists
        if (!orderService.courierExists(courierId)) {
            return ResponseEntity.status(HttpStatus.UNPROCESSABLE_ENTITY).body(Map.of(
                "error", "Invalid courier id",
                "details", "Courier with id " + courierId + " not found"
            ));
        }

        boolean isUpdated = orderService.updateOrderCourier(orderId, courierId);
        ...
        return ResponseEntity.ok(Map.of("courier_id", courierId));
    }
