...
public class OrderService {
    ...
    // Retrieves orders by courier ID
    public List<ApiOrderDTO> getOrdersByOrderStatusId(int orderStatusId) {
        List<Order> orders = orderRepository.findByOrderStatusId(orderStatusId);
        return orders.stream()
                .map(this::convertToApiOrderDTO)
                .collect(Collectors.toList());
    }
    ...
    // Checks if a status with the given ID exists
    public boolean orderStatusExists(Integer orderStatusId) {
        return courierRepository.existsById(orderStatusId);
    }
    ...
    public boolean updateOrderCourier(int orderId, int courierId) {
